...
def test_step3(browser):
    ...
    testpage.enter_title_post('Grand Theft Auto VI')
    testpage.enter_description_post('Vodka')
    testpage.enter_content_post("".join(random.choices(string.ascii_lowercase + string.digits, k=200)))
    testpage.click_save_new_post_button()
    time.sleep(testdata['sleep_time'])
    assert testpage.check_exist_post() == 'Grand Theft Auto VI'
...
def test_step4(browser):
    ...
    testpage.enter_content_field("".join(random.choices(string.ascii_lowercase + string.digits, k=100)))
